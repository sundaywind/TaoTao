 ...
 * Generated at: 2019-08-24 01:28:53 UTC
...
public final class item_002dparam_002dadd_jsp extends org.apache.jasper.runtime.HttpJspBase
  ...
  public void _jspService(final javax.servlet.http.HttpServletRequest request, final javax.servlet.http.HttpServletResponse response)
    ...
    try {
      ...
      out.write("\t\t<!-- selectItemCat：在common.js中已经实现了  -->\r\n");
      ...
      out.write("\t$(function(){ // 页面初始化\r\n");
      out.write("\t\tTAOTAO.initItemCat({ // 括号里面全是方法的一个参数\r\n");
      out.write("\t\t\t/*\r\n");
      out.write("\t\t\t\t参数的作用是参数回调，选择商品类目以后调用了fun这个方法。\r\n");
      out.write("\t\t\t\t调用fun这个方法，来判断：判断的选择的商品规格是否已添加。\r\n");
      out.write("\t\t\t*/\r\n");
      out.write("\t\t\t/*\r\n");
      out.write("\t\t\t\t这是一个方法\t\t\tK  ， V\r\n");
      out.write("\t\t\t\tTAOTAO.initItemCat({fun：Value})\r\n");
      out.write("\t\t\t*/\r\n");
      out.write("\t\t\tfun:function(node){\r\n");
      out.write("\t\t\t$(\".addGroupTr\").hide().find(\".param\").remove();\r\n");
      out.write("\t\t\t\t// ajax请求 根据返回的data 判断选择的目录是否已经添加过规格\r\n");
      ...
      out.write("\t\t\t// 生成json字符串的过程：生成的json都放进params变量中\r\n");
      out.write("\t\t\tvar params = [];\r\n");
      out.write("\t\t\tvar groups = $(\"#itemParamAddTable [name=group]\"); // 1、name=group的组选择出来\r\n");
      out.write("\t\t\t// i：当前循环的下标； e:当前循环的对象\r\n");
      out.write("\t\t\tgroups.each(function(i,e){ // 2、做循环\r\n");
      out.write("\t\t\t\tvar p = $(e).parentsUntil(\"ul\").parent().find(\"[name=param]\"); // 3、把name=param取出来，一个一个放进p中\r\n");
      out.write("\t\t\t\tvar _ps = [];\r\n");
      out.write("\t\t\t\tp.each(function(_i,_e){\r\n");
      out.write("\t\t\t\t\t// siblings：兄弟\r\n");
      out.write("\t\t\t\t\tvar _val = $(_e).siblings(\"input\").val(); // 4、遍历变量p 选择兄弟的值放入val变量中\r\n");
      out.write("\t\t\t\t\tif($.trim(_val).length>0){\r\n");
      out.write("\t\t\t\t\t\t_ps.push(_val);\t// 5、把val变量的值放入数组中\r\n");
      out.write("\t\t\t\t\t}\r\n");
      out.write("\t\t\t\t});\r\n");
      out.write("\t\t\t\tvar _val = $(e).siblings(\"input\").val();\r\n");
      out.write("\t\t\t\tif($.trim(_val).length>0 && _ps.length > 0){\r\n");
      out.write("\t\t\t\t\tparams.push({\t// 6、把组对应的值 和 项对应的值放进变量params中（这是一次循环）\r\n");
      ...
      out.write("\t\t\t// JSON.stringify(params)：将js对象转换为字符串（JSON内置的 方法）\r\n");
